...
function appendSamples(samples){
  ...
  for(const s of samples)if(s&&Array.isArray(s.legal)&&s.legal.length){s.priority=Number.isFinite(s.priority)?s.priority:1;s.age=0;replay.push(s);}for(const s of replay)if(s)s.age=(Number(s.age)||0)+1;
