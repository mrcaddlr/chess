...
async function learnerMove(c,sims,learnerBrain=brain,hist=repetition,detailed=false){const legal=c.moves({verbose:true});if(!legal.length)return detailed?{move:null,policy:[],legal:[]}:null;try{let result;if(!learnerBrain){const move=legal[Math.floor(Math.random()*legal.length)];result={move,policy:move?[{a:actionIndex(move),p:1}]:[],legal:legal.map(actionIndex)}}else{const safeSims=Math.max(1,Math.min(64,Number(sims)||4));result=await mcts(c,safeSims,learnerBrain,detailed)}const proposed=result?.move||result;const safe=safeRepetitionMove(c,proposed,learnerBrain,hist);if(safe.forcedDraw)return detailed?{move:null,policy:result?.policy||[],legal:legal.map(actionIndex)}:null;if(!safe.move)return detailed?{move:null,policy:result?.policy||[],legal:legal.map(actionIndex)}:null;retainMctsChild(c,safe.move);return detailed?{move:safe.move,policy:result?.policy||[{a:actionIndex(safe.move),p:1}],legal:result?.legal||legal.map(actionIndex),nodes:result?.nodes||0}:safe.move}catch(e){resetMctsTree();log('Learner search error: '+e.message+' · search cache reset');const safe=safeRepetitionMove(c,null,learnerBrain,hist);return detailed?{move:safe.move||null,policy:safe.move?[{a:actionIndex(safe.move),p:1}]:[],legal:legal.map(actionIndex)}:safe.move}}
