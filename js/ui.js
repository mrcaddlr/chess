...
function renderTrainingLive(){const panel=document.querySelector('.training-visual-panel');const live=v=>document.getElementById(v);if(!panel)return;panel.classList.toggle('is-training',!!training);const s=trainingLiveState||{};live('trainingLiveBadge').textContent=training?'learning':'idle';live('livePhase').textContent=training?(s.phase||'learning'):'waiting';live('liveDetail').textContent=training?(s.detail||'self-play → replay → gradient updates'):'start training to watch the learner learn';live('liveGames').textContent=games;live('liveReplay').textContent=replay.length;live('liveSteps').textContent=steps;live('liveGeneration').textContent=generation;live('liveGamesRate').textContent=trainingSpeed?trainingSpeed.toFixed(1):'0';live('liveLoss').textContent=Number.isFinite(window.lastTrainingLoss)?window.lastTrainingLoss.toFixed(4):'—';live('liveRepetition').textContent=typeof repetitionDetections==='number'?repetitionDetections:0;const workers=document.querySelector('#trainingActivityBar');if(workers)workers.style.width=training?(Math.max(8,Math.min(100,35+trainingSpeed*3))+'%'):'0%';const at=live('trainingActivityText');if(at)at.textContent=training?(s.phase==='self-play'?'playing game '+(s.game||0)+' / '+(s.totalGames||0):'network updating'):'network idle';const wt=live('trainingWorkerText');if(wt)wt.textContent=training?(s.phase==='self-play'?'ply '+(s.ply||0)+' / '+(s.maxPlies||0):(Math.max(1,Math.min(navigator.hardwareConcurrency||2,8)-1)+' workers')):'0 workers'}
