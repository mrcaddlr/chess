...
impl Mcts{
 ...
 pub fn search_with_policy(&mut self,board:&Board,network:&Network,simulations:usize)->(Option<Move>,Vec<(Move,f32)>){
   ...
   // Root Dirichlet-style exploration: inject fresh noise into priors during self-play.
   // The lightweight xorshift source keeps the native backend dependency-free.
...
impl Trainer{
 ...
 pub fn self_play_game(&mut self,max_plies:usize)->usize{
   ...
   for _ in 0..max_plies{
     ...
     let(mv,policy)=self.mcts.search_with_policy(&board,&self.network,64);let mv=mv.filter(|m|legal.contains(m)).unwrap_or(legal[0]);
   ...
   let result=result.unwrap_or(GameResult::Draw);for(board,policy)in positions{self.replay.push(PositionSample{board,policy,value:result.value_for(board.white_to_move)});}self.replay.recent(max_plies).len()
 }
 pub fn train_steps(&mut self,_steps:usize,batch_size:usize)->LossStats{
   let mut total=Gradients::zero();let take=batch_size.max(1).min(self.replay.len());if take==0{return LossStats::default();}
   let start=self.replay.len().saturating_sub(take);for s in &self.replay.all()[start..]{total.add(&self.network.gradients(&s.board,&s.policy,s.value));}
